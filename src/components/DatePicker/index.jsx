...
class DatePicker extends Component {
  componentWillMount() {
    let that = this
    // 初始化设置开始日期和结束日期
    let { defaultDate, single } = that.props
    ...
    if (single) {
      ...
      if (!defaultDate) {
        defaultDate = { startDate: sDefaultStartDate, endDate: sDefaultEndDate }
      }
      if (!defaultDate.startDate) {
        defaultDate.startDate = sDefaultStartDate
      }
      if (!defaultDate.endDate) {
        defaultDate.endDate = sDefaultEndDate
      }
    }
    /* END: 不合法默认日期处理 */

    // 设置初始状态：当前日期、面板选中日期、面板显示的月份
    that.setCurrentDate(defaultDate)
    that.setSelectedDate(defaultDate)
    that.setInputDate(defaultDate)
    ...
    that.setPickerMonths([sLastMonthId, sEndMonthId])
  ...
  render() {
    const that = this
    const { current, dateInput, pickerVisility, pickerMonths, single, style, className } = that.props

    return (
      <div className={'datepicker-wrapper ' + className} style={style}>
   ...
   */
  getDateClassName(o) {
    const { futureDisabled, selected } = this.props
  ...
  setCurrentDate(oDate) {
    ...
    dispatchAction(oActionFact.setCurrent(oDate))
  }
  setSelectedDate(oDate) {
    const { PREFIX, dispatchAction } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.setSelected(oDate))
  }
  setInputDate(oDate) {
    const { PREFIX, dispatchAction } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.setDateInput(oDate))
  }
  setPickerMonths(aMonths) {
    const { PREFIX, dispatchAction } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.setPickerMonths(aMonths))
  }
  ...
  prevMonths() {
    const { PREFIX, dispatchAction } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.prevMonths())
  }
  nextMonths() {
    const { PREFIX, dispatchAction } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.nextMonths())
  }
  confirm() {
    const { PREFIX, dispatchAction, selected } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.ok(selected))
  }
  selectDate(dateid) {
    const { PREFIX, dispatchAction, single } = this.props
    const oActionFact = DatePickerAction(PREFIX)
    dispatchAction(oActionFact.selectDate(dateid, single))
  }
